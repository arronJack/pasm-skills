...
- 各表面（HTTP / 桌面 / 脚本）只负责协议编解码与鉴权，逻辑一律调这里。

⚠️ **迁移状态（2026-09-21）**：`pasm-mcp-server` 的 `tools.py` 目前**仍是自己那份实现**
（它先于本模块存在），尚未改为转发到这里。也就是说 **MCP 与 HTTP 两个表面暂时并存两套
等价逻辑**，存在行为漂移风险。两边的关键语义已逐条对齐，但**在 MCP 侧完成委托之前，
改认知语义必须同时改两处**。把 MCP 委托过来是已登记的后续项。
